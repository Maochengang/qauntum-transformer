...
#RY 是旋转门，它对单个量子比特执行旋转。
#CRX 是受控旋转门，它对两个量子比特执行旋转，控制量子比特决定是否执行旋转。
#enc 是量子电路的一个列表，每一层电路包括旋转门和受控旋转门
...
def apply_unitaries(base_states, unitary_params, enc, qdev, n_qbs, coeffs):
    ...
    # 归一化 base states
    repeated_base = repeated_base / torch.norm(repeated_base, dim=-1, keepdim=True)
    ...
    # 归一化最终的量子态
    lcs = lcs / torch.norm(lcs, dim=-1, keepdim=True)
